    ...
    <div className='flex items-center justify-between p-4 bg-gray-700 border-b border-solid border-customOrange focus:shadow-lg focus:shadow-customOrange border'>
      <div className='flex items-center'>
        {/* Render */}
        {
          ...
          <span onClick={() => handleSetComplete(id)} className='border border-customOrange border-solid p-3 rounded-full cursor-pointer'></span>
        )}
        <p className={`pl-3 ${completed ? "line-through" : ""}`}>{title}</p>
      </div>
      <div onClick={() => handleDelete(id)} className='h-5 w-5 cursor-pointer transition-all duration-200 ease-in'>
        <MdDelete />
      </div>
    </div>
  );
